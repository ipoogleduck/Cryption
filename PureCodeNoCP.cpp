...
void encrypt() {
	...
	cryptMessage += "*";
	encryption();
	cout << "\nCopy encryption below" << endl << cryptMessage <<endl << "Click enter to continue";
...
void decrypt() {
	...
	cout << "Paste the encrypted message (ctrl/cmd v)" << endl;
	getline(cin, cryptMessage);
	decryption();
...
int main() {
	...
	do while (key == "help") {
		...
		if (key == "help") {
			...
			cout << "\n" << key;
			cout << "\nCopy key to clipboard (select and then ctrl/cmd c)" << endl;
			cout << "please share it now" << endl;
